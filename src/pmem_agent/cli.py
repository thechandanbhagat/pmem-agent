...
def find_memory_path() -> pathlib.Path:
    ...
    Resolution order (first match wins):
      ...
      3. Existing memory file — walk up; use first found
...
def cmd_init_root() -> None:
    ...
    marker.write_text(
        ...
        "# .claude/memory/project-memory.json in this directory.\n"
    ...
    gitignore_note = "Added to .gitignore" if added else "Already in .gitignore"
...
def cmd_setup() -> None:
    """Install the Claude Code subagent definition to ~/.claude/agents/."""
    src = pathlib.Path(__file__).parent / "data" / "project-memory.md"
    dest = pathlib.Path.home() / ".claude" / "agents" / "project-memory.md"
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(src, dest)
    print(f"Installed: {dest}")
    print("The 'project-memory' subagent is now available in Claude Code.")
    print()
    print("Next: cd into your project and run `pmem init-root`")
...
COMMANDS:
  setup                       Install Claude Code subagent to ~/.claude/agents/
